...
{
    class LoaderPer : Loader
    {
        static readonly Regex reg = new Regex(@"per\( 
        ...
        public LoaderPer(TextReader tr, BuilderInfo info)
            : base("pertains...", reg, tr, info)
        {
        }
        ...
        protected override void ProcessLine(Match match)
        {
            ...
            {
                if (sense1.PertainsTo == null) sense1.PertainsTo = new List<WordSense>();
                foreach (var sense2 in senses2)
                {
                    if (sense1.PertainsTo.All(ws => ws.WordSenseId != sense2.WordSenseId))
                        sense1.PertainsTo.Add(sense2);
                }
            }
        }
    }
